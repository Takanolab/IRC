...
import org.takanolab.cache.irc.CacheHelper;
...
public class MainActivity extends AndSketch implements AndGLView.IGLFunctionEvent
{
	// Log認識用タグ
	...
	HashMap<String, KGLModelData> caches = new HashMap<String, KGLModelData>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cacheUtil = new CacheHelper(this);
	...
	synchronized private KGLModelData getCreateModel(GL10 gl, int id){
		...
		try {
			InputStream cdata = null;
			KGLModelData data = null;

			if(cacheUtil.isCacheData(modelNames[id])){
				Log.d(TAG,"Cache Hit!");
				Log.d("getCreateModel","Cache Hit");
				start = System.currentTimeMillis();
				cdata = cacheUtil.getCacheData(modelNames[id]);
				end = System.currentTimeMillis();
				Log.d("3DCG Create Time (Cached)",(end-start) + "ms");
				if(cdata != null){
					Log.d(TAG,cdata.toString());
					Log.d("getCreateModel","IS NOT NULL");
				}
				return KGLModelData.createGLModel(gl, null, modelPath, cdata, 0.05f);
			}else{
				Log.d(TAG,"Cache No Hit!\nCaching...");
				start = System.currentTimeMillis();
				data = KGLModelData.createGLModel(gl, null, modelPath, modelNames[id] + ".mqo", 0.05f);
				end = System.currentTimeMillis();
				Log.d("3DCG Create Time (No Cache)",(end-start) + "ms");
				cacheUtil.setCacheData(modelNames[id], data.getInputStream());
				return data;
			}
	...
	private void drawModelDataFixation(GL10 gl,int id) throws NyARException{
		...
		long start,end;
		long enables,enablee;
		long disables,disablee;
		long draws,drawe;
		...
		gl.glRotatef(this.zrot, 0.0f,0.0f,1.0f);
		gl.glScalef(this.scale, this.scale, this.scale);
		enables = System.currentTimeMillis();
		model_data[id].enables(gl, 10.0f);
		enablee = System.currentTimeMillis();
		draws = System.currentTimeMillis();
		model_data[id].draw(gl);
		drawe = System.currentTimeMillis();
		disables = System.currentTimeMillis();
		model_data[id].disables(gl);
		end = disablee = System.currentTimeMillis();
		if(modeltimer){
			Log.d("3DCG Enable Time",modelNames[id] + ":" + (enablee - enables) + "ms");
			Log.d("3DCG Draw Time",modelNames[id] + ":" + (drawe - draws) + "ms");
			Log.d("3DCG Disables Time",modelNames[id] + ":" + (disablee - disables) + "ms");
			Log.d("3DCG Total Time",modelNames[id] + ":" + (end-start) + "ms");
		}
		modeltimer = false;
	}
